...
%>
<div id="userprofile">
	<h3><%=user.getUserName() %>'s Profile <a style="float: right; vertical-align: bottom;"
			href="/">Go to Home &gt;&gt;</a></h3>
				...
				<table class="datatbl">
					<tr>
						...
						<td style="text-align: left;" id="dashboard-stock-follower-status"><a
							id="see-details-link"
							href="/user/<%=user == null ? "" : user.getId()%>"><%=user == null ? "" : user.getUserName()%></a>'s
							follower status <a href="http://twitter.com/#!/<%=user == null ? "" : user.getUserName()%>" style="float: right" target="_blank">Twitter Page &gt;&gt;</a></td>
					</tr>
				</table></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td style="width: 25%; text-align: center;">
				$<%=Util.commaSep(user.getCash())%>
			</td>
			<td style="width: 25%; text-align: center;">
				$<%=Util.commaSep(user.getPortfolio())%>
			</td>
			<td style="width: 25%; text-align: center;">
				$<%=Util.commaSep(user.getPortfolio() + user.getCash())%>
			</td>
			<td style="width: 25%; text-align: center;">
				<% if(user.getDirection() == 1) { %>
					<img alt="" style="margin-top: 1px;" src="/images/up.png" />
				<% } else { %>
					<img alt="" style="margin-top: 1px;" src="/images/down.png" />
				<% }%>
			</td>
		</tr>
	</table>
...
<div id="userportfolio" style="margin-top: 15px;">
	<h3><%=user.getUserName() %>'s Portfolio</h3>
		<table class="datatbl">
				...
				<td>
					<table>
						<tr>
							...
							<td>
								<a href='/stock/<%=portfolio.getStockInPortfolioList().get(i).getStockId()%>'><% out.write(portfolio.getStockInPortfolioList().get(i).getStockName()); %></a>
								<br>
