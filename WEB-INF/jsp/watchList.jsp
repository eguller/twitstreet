...
<div id="user-watch-list" class="main-div">
	<table class="datatbl">
		<tr>
			<td><jsp:include page="portfolioBar.jsp"></jsp:include></td>
		</tr>
		<tr>
			<td>
				<%
						...
						<tr onmouseover="$('#watch-item-<%=stock.getId()%>').show()"
							...
							<td>
								...
								stock.getName())%>"><%=stock.getName()%>
												...
												%>
												<%=GUIUtil.getInstance().getVerifiedIcon(
									lang)%>
												<%
													}
												%>
										...
										<td align="right">
											<div id="watch-item-<%=stock.getId()%>" style="display: none">
												<a class="red-profit" href="javascript:void(0)"
													title="<%=lutil.get("watchlist.remove", lang,
								stock.getName())%>"
									...
									<tr>
										...
										<td colspan="1" rowspan="1" align="right">
											...
											%> <%=(stock.getChangePerHour() != 0) ? Util
									.getNumberFormatted(
											stock.getChangePerHour(), false,
											true, true, true, false, true)
									: "&nbsp;"%> <%
 	} else {
 %> <%=GUIUtil.getInstance()
									.getSpeedCalculation(stock, lang)%> <%
 	}
 %>
										</td>
