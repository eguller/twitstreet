...
<div id="balance" class="main-div">
	...
	<table class="right_aligned datatbl">
		<tr>
			<td><b><%=lutil.get("balance.rank", lang)%></b>
			</td>
			<td
				style="padding-left: 2px; padding-right: 2px; text-align: center;">:</td>
			<td colspan="2" id="balance_rank" style="text-align: left">
				<%
					if (user != null && user.getDirection() > 0) {
				%>
				<div id="balance_direction" style="text-align: left"><%=user == null ? "" : user.getRank()%>.<img
						src="/images/up_small.png" />
				</div> <%
 	} else if (user != null && user.getDirection() < 0) {
 %>
				<div id="balance_direction" style="text-align: left"><%=user == null ? "" : user.getRank()%>.<img
						src="/images/down_small.png" />
				</div> <%
 	} else {
 %>
				<div id="balance_direction" style="text-align: left"><%=user == null ? "" : user.getRank()%>.
				</div>
				<%-- <img src="/images/nochange_small.png" /></div> --%> <%
 	}
 %>
			</td>
		...
		<tr>
			<td><b><%=lutil.get("balance.cash", lang)%></b>
			</td>
			<td
				style="padding-left: 2px; padding-right: 2px; text-align: center;">:</td>
		...
		<tr>
			<td><b><%=lutil.get("balance.portfolio", lang)%></b>
			</td>
			<td
				style="padding-left: 2px; padding-right: 2px; text-align: center;">:</td>
		...
		<tr>
			<td><b><%=lutil.get("balance.total", lang)%></b>
			</td>
			<td
				style="padding-left: 2px; padding-right: 2px; text-align: center;">:</td>
			<td colspan="2" id="total_value" style="text-align: left">$<%=user == null ? "" : Util.commaSep(user.getCash()
					+ user.getPortfolio())%></td>
			<td />
		<tr>
			<td></td>
			<td
			...
			<%
				...
				if (user != null && user.getProfit() != 0) {
					...
					speedStr = Util.getNumberFormatted(user.getProfit(), true,
							false, true, true, false, true);
				}
			%>
			<td colspan="2" title="<%=lutil.get("balance.speed.tip", lang)%>"
				id="total_trend" style="text-align: left"><%=speedStr%></td>
			<td />
		</tr>
	</table>
