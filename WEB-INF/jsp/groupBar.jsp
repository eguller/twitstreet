...
<div class="main-tabs" style="width: 100%;">
	<div class="title-bar">
			...
			onclick="reloadIfHashIs('#!grouplist'); loadTitle('<%=lutil.get("groups.list.title",lang)%>');">
			<%=lutil.get("grouplist.header", lang)%> </a>
<%	if(user!=null){%>
			
			<a class="my-groups-tab <%=(!selectedTab.equalsIgnoreCase("my-groups-tab"))?"": "youarehere"%>"
			onclick="reloadIfHashIs('#!mygroups'); loadTitle('<%=lutil.get("groups.mygroups.title",lang)%>');">
			<%=lutil.get("group.mygroups", lang)%> </a>
<% }%>
<%if(group!=null){ %>
			<a class="group-details-tab <%=(!selectedTab.equalsIgnoreCase("group-details-tab"))?"": "youarehere"%>"
			onclick="reloadIfHashIs('#!group=<%=group.getId()%>'); loadTitle('<%=lutil.get("groups.details.title", lang, group.getName())%>');">
			 <%=group.getName()%> 
