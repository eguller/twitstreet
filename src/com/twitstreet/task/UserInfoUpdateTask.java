	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				while (idList.size() > 0) {
					List<Long> subList = idList.subList(0,Math.min(TwitterProxyImpl.IDS_SIZE,idList.size()));
					...
					for (twitter4j.User twitterUser : twitterUserList) {
						...
						updatedUser.setUrl(twitterUser.getURL() == null ? null : twitterUser.getURL().toExternalForm());
